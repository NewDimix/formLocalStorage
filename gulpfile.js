...
function css() {
	...
	return gulp
		.src(["node_modules/normalize.css/normalize.css", path.app.style])
...
function js() {
	return (
		gulp
			.src(["app/vendors/jquery.min.js", "app/vendors/chosen.jquery.js", path.app.js, path.app.blocksJs])
			.pipe(sourcemaps.init())
			//        .pipe(uglify())
			.pipe(sourcemaps.write())
...
function jsProd() {
	return gulp.src("dist/js/*.js").pipe(uglify()).pipe(gulp.dest(path.dist.js));
}
gulp.task(jsProd);
